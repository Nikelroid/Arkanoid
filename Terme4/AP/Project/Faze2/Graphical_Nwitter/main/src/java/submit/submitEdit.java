...
public class submitEdit {
    ...
    public void editUsername(String username, String newuser) throws IOException {
        for(int i = 0; i < users.size(); i++)
            if (users.get(i).getUsername().equals(username)) {
        ...
        Path newPath = Paths.get(simple.toAbsolutePath().getParent()+
                "\\main\\src\\resources\\profiles\\"+newuser+".png");
        File file2 = new File(newPath.toString());
        ...
        if (!success) {
            JOptionPane.showMessageDialog(null,"Chamge image failed, select again from home");
        }
